...
class CosineStats():
    def __init__(self, layer, population_size, basic_mode=False, reference_layer=None):
        ...
        if not self.basic_mode:
            ...
            if self.reference_layer is not None:
                w_ref = np.array(torch.cat((self.reference_layer.weight.data, self.reference_layer.bias.data.unsqueeze(1)), dim=1).clone().cpu())
                self.reference_mean_w = self._compute_mean_w_population(w_ref)
    ...
    def initial_stats(self):
        ...
        if not self.basic_mode:
            ...
            if self.reference_layer is not None:
                ...
                cm = cosine_similarity(self.reference_mean_w, mean_w).diagonal()
        else:
            cd = []
            cm = []
